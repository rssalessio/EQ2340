...
%Assignment3: Alessio Russo and Lars Lindemann
	...
	% initialize outputs and help matrix
	%beta is used only at the beginning of the algorithm, no need to allocate a new variable
	...
	for t=nT-1:-1:1
		for i=1:nS
			if fD %sum over N by omitting "end" state in a
				sumS        = sum(a(i,1:end-1).*b(:,t+1).'.*betaHat(:,t+1).');
			else %sum over N
				sumS        = sum(a(i,:).*b(:,t+1).'.*betaHat(:,t+1).');
			end
			betaHat(i,t)= sumS / c(t);
		end
	end
